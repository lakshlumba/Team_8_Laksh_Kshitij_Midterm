 ...
 * @author Laksh
 */
object MusicYearPerdictREG {
  
    def main(args: Array[String]) {
      
        // Load training data in LIBSVM format.
        val sc = new SparkContext(new SparkConf().setAppName("MusicYearPerdict_Reg"))
        
        val data = sc.textFile("C:/Users/Laksh/Desktop/Bigdata/midterm/YearPredictionMSD_R.csv")
        ...
        val parsedData = data.map { line =>
          ...
          LabeledPoint(parts(0).toDouble, normalizer1.transform(Vectors.dense(myList.map(_.toDouble)))) 
        }
